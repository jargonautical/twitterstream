...
* Use the coordinates field, if it exists, to geocode the tweet. This method gives the most reliable location information, but unfortunately not very many tweets contain this field and you have to figure out some way of translating the coordinates into a state
* Use the place field. This field is hand-entered by the twitter user and may not always be present, and will not typically contain a state. It might contain a city, though, which you could use to determine the state.
